...
class BuildTarget(Target):
  ...
  __slots__ = ('_name', '_func', '_kwargs', '_scope', '_tags', '_deps', '_includes', '_staged', '_rule_name')
  def __init__(self, name: references.Target,
...
class StagedBuildTargetImpl(threading.GraphNode, StagedBuildTarget):
  ...
  def LoadToTemp(self, package_dir: str, binary_dir: str) -> tuple[str, dict[str, str], packaging.ExportablePackage]:
    ...
    try:
      ...
      try:
        ...
        if os.path.exists(filename):
          ...
          raise exceptions.BuildDefsRaisesException(target_name, buildrule_type, highlighted) from None
      except:
        pass
  ...
  def run_job(self, debug: bool, internal_access: threading.UpdateGraphResponseData | None = None) -> typing.Any:
